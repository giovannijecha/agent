...
type TransportOpen = OpenCodeZenTransport["open"];
type OwnedTransportStream = Readonly<{
  close: OpenCodeZenTransportStream["close"];
  contentType: string | undefined;
  read: OpenCodeZenTransportStream["read"];
  statusCode: number;
}>;

function createError(kind: OpenCodeZenCreateError["kind"]): OpenCodeZenCreateError {
...
function validTransportErrorKind(
  value: unknown,
): value is OpenCodeZenTransportErrorKind {
  return (
    value === "cancelled" ||
    value === "closed" ||
...
function transportResult<T>(
  value: unknown,
): Result<T, OpenCodeZenTransportError> | undefined {
  try {
...
function wireReason(error: WireError): OpenCodeZenFailureReason {
...
function snapshotTransportStream(value: unknown): OwnedTransportStream | undefined {
  try {
    ...
    const candidate = value as OpenCodeZenTransportStream;
    ...
    return Object.freeze({
      close: close.bind(value) as OpenCodeZenTransportStream["close"],
      contentType,
      read: read.bind(value) as OpenCodeZenTransportStream["read"],
      statusCode,
    });
  } catch (_cause: unknown) {
    return undefined;
...
class OpenCodeZenStream implements ModelStream<OpenCodeZenError> {
  ...
  read(): Promise<Result<ModelStreamEvent, OpenCodeZenError>> {
  ...
  async close(): Promise<Result<void, OpenCodeZenError>> {
    if (this.#closed) {
      return ok(undefined);
    }
    this.#closed = true;
  ...
  async #read(): Promise<Result<ModelStreamEvent, OpenCodeZenError>> {
    try {
      while (!this.#closed) {
        ...
        let received: Result<Uint8Array | null, OpenCodeZenTransportError> | undefined;
        try {
...
export class OpenCodeZenModel implements StreamingModel<OpenCodeZenError> {
  ...
  static create(
    ...
    if (
      ...
      instructions.length > OPENCODE_ZEN_LIMITS.instructionsCodeUnits ||
      /[\u0000-\u0008\u000B\u000C\u000E-\u001F\u007F-\u009F]/u.test(instructions)
    ) {
  ...
  async open(
  ...
  ): Promise<Result<ModelStream<OpenCodeZenError>, OpenCodeZenError>> {
    ...
    const request: OpenCodeZenTransportRequest = Object.freeze({
      body: body.value,
    });
    let opened: Result<OpenCodeZenTransportStream, OpenCodeZenTransportError> | undefined;
    try {
      opened = transportResult<OpenCodeZenTransportStream>(
    ...
    return ok(new OpenCodeZenStream(stream));
  }
}
