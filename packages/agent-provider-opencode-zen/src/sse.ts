...
export class SseDecoder {
  ...
  push(text: string): Result<void, SseError> {
    ...
    if (
      ...
      OPENCODE_ZEN_LIMITS.sseBufferCodeUnits
    ) {
  ...
  next(): Result<SseRead, SseError> {
    ...
    while (true) {
      ...
      this.#eventLines += 1;
      if (
        this.#totalLines > OPENCODE_ZEN_LIMITS.sseTotalLines ||
        this.#eventLines > OPENCODE_ZEN_LIMITS.sseLinesPerEvent
      ) {
        return err(failure("limit"));
      }
      ...
      if (field === "data") {
        ...
        if (this.#dataCodeUnits > OPENCODE_ZEN_LIMITS.sseDataCodeUnits) {
          return err(failure("limit"));
        }
