...
import type { OpenCodeZenModelId } from "./models.js";
...
export function encodeRequest(
  ...
  try {
    ...
    return body.length <= OPENCODE_ZEN_LIMITS.requestCodeUnits
      ? ok(body)
      : err(failure("limit"));
...
export class ChatCompletionDecoder {
  ...
  accept(data: string): Result<readonly ModelStreamEvent[], WireError> {
    ...
    this.#wireEvents += 1;
    if (this.#wireEvents > OPENCODE_ZEN_LIMITS.wireEvents) {
    ...
    if (data.length > OPENCODE_ZEN_LIMITS.sseDataCodeUnits) {
      return err(failure("limit"));
    }
  ...
  #acceptToolFragment(value: unknown): Result<void, WireError> {
    if (
      !Array.isArray(value) ||
      value.length === 0 ||
      value.length > OPENCODE_ZEN_LIMITS.toolCallsPerBatch
    ) {
      return err(failure("protocol"));
    }
    ...
    for (const fragment of value) {
      ...
      if (
        ...
        index < 0 ||
        index >= OPENCODE_ZEN_LIMITS.toolCallsPerBatch ||
        seen.has(index) ||
      ...
      if (functionFragment !== undefined && functionFragment !== null) {
        ...
        if (argument !== undefined && argument !== null) {
          ...
          if (
            assembly.argumentCodeUnits > OPENCODE_ZEN_LIMITS.toolArgumentCodeUnits ||
            this.#toolArgumentCodeUnits >
              OPENCODE_ZEN_LIMITS.toolBatchArgumentCodeUnits
          ) {
            return err(failure("limit"));
          }
      ...
      if (
        ...
        (index === this.#tools.length && this.#tools.length >=
          OPENCODE_ZEN_LIMITS.toolCallsPerBatch)
      ) {
        return err(failure("protocol"));
      }
