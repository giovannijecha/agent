...
class FakeStream implements OpenCodeZenTransportStream {
  readonly contentType: string | undefined;
  readonly statusCode: number;
  readonly #chunks: Result<Uint8Array | null, OpenCodeZenTransportError>[];
  closeCalls = 0;
  closeResult: Result<void, OpenCodeZenTransportError> = ok(undefined);

  constructor(
    chunks: Result<Uint8Array | null, OpenCodeZenTransportError>[],
  ...
  async read(): Promise<Result<Uint8Array | null, OpenCodeZenTransportError>> {
    return this.#chunks.shift() ?? ok(null);
  }

  async close(): Promise<Result<void, OpenCodeZenTransportError>> {
    this.closeCalls += 1;
    return this.closeResult;
  }
}

class FakeTransport implements OpenCodeZenTransport {
  readonly #opened: Result<OpenCodeZenTransportStream, OpenCodeZenTransportError>;
  cancellation: CancellationSignal | undefined;
  request: OpenCodeZenTransportRequest | undefined;

  constructor(
    opened: Result<OpenCodeZenTransportStream, OpenCodeZenTransportError>,
  ) {
    this.#opened = opened;
  }
...
class SequenceFakeTransport implements OpenCodeZenTransport {
  readonly requests: OpenCodeZenTransportRequest[] = [];
  readonly #streams: OpenCodeZenTransportStream[];

  constructor(streams: OpenCodeZenTransportStream[]) {
...
async function open(
  provider: OpenCodeZenModel,
  cancellation: Cancellation = new Cancellation(),
  tools: readonly ToolDescriptor[] = [],
): Promise<ModelStream<OpenCodeZenError>> {
...
async function read(
  stream: ModelStream<OpenCodeZenError>,
...
test("validates construction without retaining hostile values", () => {
  const invalidInstructions = OpenCodeZenModel.create(
  ...
  const invalidTransport = OpenCodeZenModel.create(
    hostile as unknown as OpenCodeZenTransport,
    "You are agent.",
  );
...
test("encodes the fixed model, instructions, conversation, and exact tool schema", async () => {
  ...
  assert.equal(parsed.model, OPENCODE_ZEN_MODEL);
...
test("converges a multi-part edit through separate concrete provider decisions", async () => {
  ...
  const provider = OpenCodeZenModel.create(
    transport,
...
test("rejects a concurrent read and closes idempotently", async () => {
  const pending = new Deferred<Result<Uint8Array | null, OpenCodeZenTransportError>>();
  let readCalls = 0;
  let closeCalls = 0;
  const transportStream: OpenCodeZenTransportStream = {
...
test("bounds SSE data without retaining model content", async () => {
  ...
  const oversized =
    "data: " +
    privatePrefix +
    "x".repeat(OPENCODE_ZEN_LIMITS.sseDataCodeUnits + 1) +
    "\n\n";
...
test("honors cancellation before transport open", async () => {
  let calls = 0;
  const transport: OpenCodeZenTransport = {
    async open(): Promise<Result<OpenCodeZenTransportStream, OpenCodeZenTransportError>> {
      calls += 1;
      return err(Object.freeze({ kind: "connection" }));
    },
  };
  const created = OpenCodeZenModel.create(transport, "You are agent.");
  assert.ok(created.ok);
