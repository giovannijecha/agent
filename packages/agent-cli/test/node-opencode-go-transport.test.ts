...
import {
  type HttpsClient,
  NodeOpenCodeGoTransport,
  OPENCODE_GO_CHAT_PATH,
  OPENCODE_GO_ORIGIN,
  OPENCODE_GO_TRANSPORT_LIMITS,
} from "../dist/node-opencode-go-transport.js";
...
function createTransport(client: HttpsClient): NodeOpenCodeGoTransport {
  const created = NodeOpenCodeGoTransport.create("valid-value", client);
  assert.ok(created.ok);
  return created.value;
}

test("rejects invalid credentials and hostile HTTPS clients without throwing", () => {
  assert.equal(NodeOpenCodeGoTransport.create("", new FakeClient()).ok, false);
  ...
  assert.deepEqual(NodeOpenCodeGoTransport.create("valid-value", hostile as never), {
...
test("uses only the admitted HTTPS origin, path, headers, and request body", async () => {
  ...
  assert.ok(opened.ok);
  assert.equal(OPENCODE_GO_ORIGIN, "https://opencode.ai");
  assert.equal(client.options?.hostname, "opencode.ai");
  assert.equal(client.options?.path, OPENCODE_GO_CHAT_PATH);
  ...
  assert.equal(client.options?.maxHeaderSize, OPENCODE_GO_TRANSPORT_LIMITS.headerBytes);
...
test("rejects concurrent reads and oversized response chunks", async () => {
  ...
  response.emit(
    "data",
    new Uint8Array(OPENCODE_GO_TRANSPORT_LIMITS.responseChunkBytes + 1),
  );
