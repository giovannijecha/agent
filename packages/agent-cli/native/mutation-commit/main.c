...
int main(int argument_count, char **arguments) {
  ...
  if (!agent_mutation_read_request(stdin, &request)) {
    return 1;
  }
  const enum agent_mutation_status status = agent_mutation_commit(&request);
  agent_mutation_dispose_request(&request);
  return agent_mutation_write_response(stdout, status) ? 0 : 1;
}
