...
} from "./platform-workspace-namespace-protocol.js";
import type {
...
const NAMESPACE_STDIO = Object.freeze([
  "pipe",
  "pipe",
  "pipe",
  "pipe",
...
export const PLATFORM_WORKSPACE_NAMESPACE_DEADLINES = Object.freeze({
  cleanupMilliseconds: 250,
  operationMilliseconds: 5_000,
});

export type PlatformWorkspaceNamespaceBoundary = Readonly<{
  launch(
    executable: string,
...
export type PlatformWorkspaceNamespaceCreateError = Readonly<{
...
function brokerPath(
  ...
  const executable = platform === "win32"
    ? "agent-namespace-commit.exe"
    : "agent-namespace-commit";
  return path.join(
    packageRoot(),
    ".native-build",
...
const platformWorkspaceNamespaceBoundary: PlatformWorkspaceNamespaceBoundary =
  Object.freeze({
...
/** Exact native commit adapter for one approved namespace effect. */
export class PlatformWorkspaceNamespaceCommitter
implements WorkspaceNamespaceCommitter {
  ...
  static create(
    ...
    architecture: string,
    boundary: PlatformWorkspaceNamespaceBoundary =
      platformWorkspaceNamespaceBoundary,
  ): Result<PlatformWorkspaceNamespaceCommitter, PlatformWorkspaceNamespaceCreateError> {
    if (
  ...
  commit(
    ...
    const encoded = encodePlatformWorkspaceNamespace(request);
    if (!encoded.ok) {
  ...
  #observe(
    child: ChildProcess,
    frame: Uint8Array,
    requestKind: WorkspaceNamespaceCommit["kind"],
    ...
    return new Promise((resolve) => {
      ...
      const settle = (
        result: Result<WorkspaceNamespaceCommitResult, ToolHandlerError>,
      ): void => {
        if (settled) {
          return;
        }
      ...
      const stop = (failure: ToolHandlerError): void => {
        ...
        try {
          cancelCleanupDeadline = this.#boundary.schedule(
            () => settle(err(failure)),
            PLATFORM_WORKSPACE_NAMESPACE_DEADLINES.cleanupMilliseconds,
          );
        } catch (_cause: unknown) {
          settle(err(failure));
          return;
      ...
      child.stdout.on("data", (chunk) => {
        ...
        const remaining =
          PLATFORM_WORKSPACE_NAMESPACE_LIMITS.responseBytes - outputLength;
      ...
      child.once("close", (code, signal) => {
        ...
        const decoded = decodePlatformWorkspaceNamespaceResponse(output);
        if (!decoded.ok) {
          settle(err(toolFailure("io")));
        } else if (decoded.value.kind === "failure") {
          settle(err(toolFailure(decoded.value.error)));
        } else if (
          (requestKind === "create_directory" &&
            decoded.value.result !== "directory_created") ||
          (requestKind === "move" && decoded.value.result !== "moved") ||
          (requestKind === "remove" && decoded.value.result !== "removed")
        ) {
          settle(err(toolFailure("io")));
        } else {
      ...
      try {
        cancelOperationDeadline = this.#boundary.schedule(
          () => stop(toolFailure("io")),
          PLATFORM_WORKSPACE_NAMESPACE_DEADLINES.operationMilliseconds,
        );
      } catch (_cause: unknown) {
        stop(toolFailure("io"));
      }
