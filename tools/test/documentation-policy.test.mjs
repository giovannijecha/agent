...
test("binds the dormant durable credential boundary", () => {
  ...
  for (const marker of [
    ...
    /exact production CLI Node and native platform authorities/u,
    ...
    /direct\s+`node:fs` or `node:child_process` authority/u,
    /native platform\s+authority retains the exact ordered C\/H path set/u,
    /complete UTF-8 source after\s+only CRLF-to-LF normalization and use SHA-256/u,
    /complete source\s+snapshots are the verifier's sole authority/u,
    /does not execute product code or\s+infer commands, exports, aliases, assignments, or general capability flow/u,
    /legitimate edit requires review of the complete authority-bearing source/u,
    /unfamiliar syntax and split\s+command or path fragments cannot bypass the boundary/u,
  ...
  for (const [file, marker] of [
    ...
    ["docs/ENGINEERING.md", "reviewed\\s+module specifier, import bindings, and normalized"],
    ["docs/ENGINEERING.md", "native\\s+C/H platform tree"],
    ["docs/ENGINEERING.md", "normalizes only CRLF to LF"],
    ["docs/ENGINEERING.md", "exact source digest is the sole verifier authority"],
    ["docs/ENGINEERING.md", "partial command, export, alias, assignment, or"],
    ...
    ["docs/MAINTENANCE.md", "exact module specifier, imported bindings, and SHA-256"],
    ["docs/MAINTENANCE.md", "exact ordered C/H path set"],
