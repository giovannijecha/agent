...
test("binds the dormant durable credential boundary", () => {
  ...
  for (const marker of [
    ...
    /direct-alias grammar admits at most 32\s+nested balanced parentheses or legacy angle-bracket assertions/u,
    /Assertion types are limited to\s+`const`, `typeof` plus a qualified identifier/u,
    /any\s+other assertion type fails closed/u,
    /named runtime binding\s+literally called `type` remains runtime/u,
    /Identifiers are tokenized by\s+Unicode code point through the ECMAScript\s+`ID_Start` and `ID_Continue`/u,
    /escaped identifiers fail closed/u,
    /complete\s+default-export expression must be\s+a direct alias/u,
    /lexically\s+unambiguous\s+automatic-semicolon boundaries/u,
    /Multiline calls, member access, tagged\s+templates, and admitted transparent assertions remain continuations/u,
    /never\s+executes\s+product\s+code\s+or\s+claims\s+general\s+capability-flow\s+analysis/u,
  ...
  for (const [file, marker] of [
    ...
    ["docs/ENGINEERING.md", "module-scope\\s+direct identifier aliases"],
    ["docs/ENGINEERING.md", "Module-scope object and"],
    ["docs/ENGINEERING.md", "`var`\\s+declaration encountered while parentheses"],
    ["docs/ENGINEERING.md", "grammar admits at most 32 nested balanced parentheses"],
    ["docs/ENGINEERING.md", "every other\\s+assertion type fails\\s+closed"],
    ["docs/ENGINEERING.md", "runtime binding literally called `type`"],
    ["docs/ENGINEERING.md", "Identifiers are tokenized by Unicode code point"],
    ["docs/ENGINEERING.md", "escaped identifiers fail closed"],
    ["docs/ENGINEERING.md", "complete delimited expression"],
    ...
    ["docs/MAINTENANCE.md", "module-scope object or array binding pattern"],
    ["docs/MAINTENANCE.md", "`var` declarations in blocks, loops, and functions"],
    ["docs/MAINTENANCE.md", "parenthesized bounded `as` and `satisfies` chains"],
    ["docs/MAINTENANCE.md", "assertion types outside the bounded alias\\s+grammar"],
    ["docs/MAINTENANCE.md", "32-level direct-alias depth bound"],
    ["docs/MAINTENANCE.md", "runtime binding literally named\\s+`type`"],
    ["docs/MAINTENANCE.md", "Unicode aliases in both the basic and astral"],
    ["docs/MAINTENANCE.md", "escaped identifiers"],
    ["docs/MAINTENANCE.md", "complete default-export expression"],
