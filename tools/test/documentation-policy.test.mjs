...
test("binds the dormant durable credential boundary", () => {
  ...
  for (const marker of [
    ...
    /Exported object and array binding patterns fail closed/u,
    /complete default-export expression must be a direct alias/u,
    /lexically\s+unambiguous automatic-semicolon boundaries/u,
    /Multiline calls, member\s+access, tagged templates, and\s+transparent assertions remain\s+continuations/u,
    /never executes\s+product code or claims\s+general\s+capability-flow analysis/u,
  ...
  for (const [file, marker] of [
    ...
    ["docs/ENGINEERING.md", "array binding patterns are outside"],
    ...
    ["docs/MAINTENANCE.md", "exported object and array binding patterns"],
