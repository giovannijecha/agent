...
test("binds the dormant durable credential boundary", () => {
  ...
  for (const marker of [
    ...
    /Identifiers are tokenized by\s+Unicode code point through the ECMAScript `ID_Start` and `ID_Continue`/u,
    /escaped identifiers fail closed/u,
    /complete default-export expression must be\s+a direct alias/u,
    /lexically\s+unambiguous\s+automatic-semicolon boundaries/u,
    /Multiline calls, member\s+access,\s+tagged templates, and\s+transparent assertions\s+remain continuations/u,
  ...
  for (const [file, marker] of [
    ...
    ["docs/MAINTENANCE.md", "runtime binding literally named `type`"],
