...
test("binds the dormant durable credential boundary", () => {
  ...
  for (const marker of [
    ...
    /complete dormant CLI product\s+tree and native platform authority each retain their exact ordered path set/u,
    /digest forms cover complete\s+UTF-8 source after only CRLF-to-LF normalization and use SHA-256/u,
    /complete source\s+snapshots are the verifier's sole authority/u,
    /does not execute\s+product code or infer\s+partial strings, commands, exports, aliases, assignments, or general capability\s+flow/u,
    /legitimate edit requires review of the complete owning source tree/u,
  ...
  for (const [file, marker] of [
    ...
    ["docs/MAINTENANCE.md", "complete dormant CLI product tree and native platform"],
